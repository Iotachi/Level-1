...
# If item is unique, store its result number and set to initialized
...
data modify entity @e[type=minecraft:item,nbt={Item:{tag:{urng:0b}}},limit=1] Item.tag.urng set value 1b
...
execute if score $results code < @s code run function game:stage7/unique_rng

# Sets $results' code score to 0 for future rng
execute if score $results code = @s code run kill @s
